...
const port=4193,origin=`http://127.0.0.1:${port}`,db=await database(),owner=await user(db),stub=mockSupabase(db);stub.users.set('fixture-only',owner);globalThis.fetch=stub.fetch;
...
const server=createServer(async(req,res)=>{try{const url=new URL(req.url,origin),json=(data,status=200)=>{res.writeHead(status,{'content-type':'application/json','cache-control':'no-store'});res.end(JSON.stringify(data).replaceAll('http://supabase.test/storage/v1',origin+'/__storage'));};
 ...
 if(url.pathname.startsWith('/__storage/')){
  ...
  if(upload&&req.method==='PUT'){const chunks=[];for await(const c of req)chunks.push(c);const bytes=Buffer.concat(chunks);stub.media.set(key,{size:bytes.length,contentType:'video/mp4',bytes});await mkdir(join(fixtureRoot,'work/product-studio/fixture-renders'),{recursive:true});await writeFile(join(fixtureRoot,'work/product-studio/fixture-renders',key.split('/').at(-1)),bytes);return json({ok:true});}
 ...
 if(url.pathname==='/api/studio'||url.pathname==='/api/studio-worker'){
  ...
  const r=await(url.pathname==='/api/studio-worker'?worker(context):method==='GET'?getStudio(context):postStudio(context));return json(await r.json(),r.status);
 }
