...
test('isolated runtime blocks filesystem escape and network; actual video-use render has captions and reviewed output', {skip:!python},async()=>{
 const root=await mkdtemp(resolve(tmpdir(),'video-use-test-'));try{
  ...
  const fake=async(url,opts)=>{const request=JSON.parse(opts.body);assert.equal(request.model,CHAT_MODEL);if(request.tools[0]?.name!=='observations')assert.match(request.input[0].content,/Video Use/);if(n===1)assert.match(JSON.stringify(request.input.at(-1)),/Verify the current render/);const [name,args]=request.tools[0]?.name==='observations'?['observations',{message:'All fixture windows preserve the requested content.'}]:calls[n++];return new Response('data: '+JSON.stringify({model:CHAT_MODEL,usage:{prompt_tokens:100,completion_tokens:20},choices:[{delta:{tool_calls:[{index:0,id:'call'+n,function:{name,arguments:JSON.stringify(args)}}]},finish_reason:'tool_calls'}]})+'\n\ndata: [DONE]\n\n');};
  const result=await runVideoUse({root,mode:'edit',request:'Test fixture',strategy:'Preserve two spoken words with captions; synthetic integration fixture.',sources:media,transcripts:transcript,env:{OPENAI_API_KEY:'fixture-astra',REFERENCE_FLASH_KEY:'test'},fetchImpl:fake,sandboxOptions:{python}});
