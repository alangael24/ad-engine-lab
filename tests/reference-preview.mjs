...
globalThis.fetch=async(input,opts)=>{const url=typeof input==='string'?input:input.url;if(url.startsWith('http://supabase.test/'))return stub.fetch(input,opts);if(live&&(url.startsWith('https://opencode.ai/zen/go/v1/')||url==='https://api.elevenlabs.io/v1/speech-to-text'))return realFetch(input,opts);throw Error('Unexpected fixture network request');};
...
createServer(async(req,res)=>{try{const url=new URL(req.url,origin);let response;
 if(url.pathname==='/__test__/sdk.js')response=new Response(sdk,{headers:{'content-type':'text/javascript'}});
 ...
 else if(['/api/studio','/api/reference-analysis'].includes(url.pathname)){
  ...
  const context={env,request};response=await(url.pathname==='/api/studio'?(req.method==='GET'?getStudio(context):postStudio(context)):(req.method==='GET'?getReferenceAnalysis(context):postReferenceAnalysis(context)));
