...
import {seedExistingAd} from './helpers/seed-existing-ad.mjs';
...
const blank=process.env.STUDIO_PREVIEW_BLANK==='true';
if(blank)env.PRODUCTION_ENABLED='false';
const f=blank?{owner:await user(db,0,0),project:null,planCalls:()=>0}:await nebulaProduction(db,stub,fixtureRoot,env,network);if(process.env.STUDIO_PREVIEW_EXISTING==='true'){await seedExistingAd(db,f);env.PRODUCTION_ENABLED='false';}
stub.users.set('fixture-only',f.owner);await mkdir(output,{recursive:true});
globalThis.fetch=async(input,options)=>{
 const url=String(input instanceof Request?input.url:input);if(url.startsWith('http://supabase.test'))return stub.fetch(input,options);
 const response=await network(input,options);
 if(process.env.STUDIO_PREVIEW_EXISTING==='true'&&url.includes('opencode.ai')){
  const body=JSON.parse(options.body),imageCount=body.messages.flatMap(m=>Array.isArray(m.content)?m.content:[]).filter(c=>c.type==='image_url').length;
  await writeFile(join(output,'flash-request-audit.json'),JSON.stringify({imageCount,model:body.model,status:response.status,bytes:options.body.length}));
  if(!response.ok)await writeFile(join(output,'flash-error.txt'),(await response.clone().text()).slice(0,2000));
 }
 return response;
};
const sdk=`export function createClient(){return {auth:{getSession:async()=>({data:{session:{access_token:'fixture-only',user:{id:'${f.owner.id}',email:'${f.owner.email}'}}}}),onAuthStateChange:()=>({data:{subscription:{unsubscribe(){}}}}),signOut:async()=>({error:null})}}}`;
...
const server=createServer(async(req,res)=>{try{
 ...
 if(u.pathname==='/__test__/info')return json({projectId:f.project?.id||null,planCalls:f.planCalls()});
 ...
 const routes={'/api/store-import':{POST:postStoreImport},'/api/studio':{GET:getStudio,POST:postStudio},'/api/studio-chat':{GET:getStudioChat,POST:postStudioChat},'/api/studio-production':{GET:getProduction,POST:postProduction},'/api/studio-production-worker':{POST:productionEndpoint},'/api/studio-worker':{POST:renderEndpoint}};
...
if(!blank)setInterval(async()=>{if(rendering)return;rendering=true;try{const {job}=await render('claim');if(job)await processRender(job,render);}catch(e){console.error('fixture_render_error',e.code||e.message);}finally{rendering=false;}},1000);
console.log(`${origin}/estudio/${f.project?'?project='+f.project.id:''}`);
