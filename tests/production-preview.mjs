...
const root=resolve(import.meta.dirname,'..'),output=join(fixtureRoot,'outputs/production-flow'),db=await database(),stub=mockSupabase(db),network=globalThis.fetch;
const port=4207,origin=`http://127.0.0.1:${port}`,env={SUPABASE_URL:'http://supabase.test',SUPABASE_SERVICE_ROLE_KEY:'test-only',GENERATION_ENABLED:'false',PRODUCTION_ENABLED:'true',REFERENCE_ANALYSIS_ENABLED:'true',PRODUCTION_WORKER_TOKEN:'fixture-production-worker-only-1234567890',STUDIO_WORKER_TOKEN:'fixture-render-worker-only-1234567890',REFERENCE_FLASH_KEY:execFileSync('security',['find-generic-password','-s','codex-opencode-api-key','-w'],{encoding:'utf8'}).trim()};
...
const server=createServer(async(req,res)=>{try{
 ...
 if(routes[u.pathname]){const chunks=[];for await(const c of req)chunks.push(c);const request=new Request(u,{method:req.method,headers:req.headers,...(req.method==='POST'?{body:Buffer.concat(chunks)}:{})});const handler=routes[u.pathname][req.method];if(!handler)return json({},405);const r=await handler({request,env});return json(await r.json(),r.status);}
