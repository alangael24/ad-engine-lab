...
export function mockSupabase(db) {
  ...
  async function fetchMock(input, options={}) {
    ...
    if(path.startsWith('/storage/v1/object/info/')) {
      const key=path.slice('/storage/v1/object/info/'.length); return media.has(key)?respond(media.get(key)):respond({message:'Not found',statusCode:'404',error:'not_found'},404);
    }
