...
test('failed production refreshes the saved storyboard before a customer retries',async()=>{
 ...
 try{
  ...
  await panel.open(project);const button=elements.find(e=>e.tag==='button');assert.equal(button.textContent,'Reintentar');assert.equal(refreshes,1);assert.equal(elements.find(e=>e.tag==='video').src,'https://media.test/approved.mp4');assert.equal(elements.find(e=>e.textContent==='Descargar video').hidden,false);
