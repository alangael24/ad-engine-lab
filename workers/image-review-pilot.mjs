...
export async function runImageReviewPilot({packets,review,save,load,auditRate,auditSeed,modelIds}){
 ...
 async function call(role,stage,subset){
  ...
  if(record){try{validate(record);}catch(error){calls.push({key,model,stage,reused,usage:record.usage??null,costUsd:record.costUsd??null,error:'invalid_cached_review'});throw error;}}
   ...
   catch(error){await save(key,{requestHash:hash(request),error:'review_transport_failed',usage:error.usage??null,costUsd:error.costUsd??null});calls.push({key,model,stage,reused:false,usage:error.usage??null,costUsd:error.costUsd??null,error:'review_transport_failed'});throw error;}
 ...
 for(const p of packets){
  ...
  catch{primaryUnavailable=true;primary={sceneId:p.contract.sceneId,checks:p.contract.criteria.map(c=>({criterionId:c.id,status:'unverifiable',difference:'unverifiable',observation:'Primary reviewer did not return a valid assessment. This is a controller placeholder, not a visual finding.',consequence:''}))};}
  ...
  const escalation=imageEscalation(p.contract,primary,{audit});
 ...
 // into ONE bounded collection request, without expanding any criterion scope.
 ...
 let secondary=[];if(secondaryPackets.length)try{secondary=await call('secondary','collection_gate',secondaryPackets);}catch{}
 const style=[];
 ...
 const decisions=states.map(s=>decideImage(s.packet.contract,s.primary,{confirmed:s.confirmed,style:style.find(x=>x.sceneId===s.packet.contract.sceneId)?.checks||[],audit:s.audit}));
