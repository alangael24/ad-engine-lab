...
Object.assign(edlSchema.properties,{sourceCaptionScenes:{type:'array',maxItems:24,items:str},captionFadeMs:{type:'integer',minimum:0,maximum:100},captionHighlight:{type:'boolean'}});
...
export async function finishSolLunaProject({root,project,shots,narration,words,base=null,cacheDirectory,cacheNamespace,strategy='',env=process.env,signal,fetchImpl=fetch,call=editorialCall,prepare=prepareSimpleSources,boards=sourceBoards,render=renderSimpleAd,evidence=buildReviewEvidence,auditCaptions=verifyCaptionFindings,onUsage}){
 ...
 try{
  ...
  if(base&&project.data.editing?.scoped){
   ...
   if(Object.keys(instructions).length){
    ...
    for(let attempt=0;attempt<2;attempt++){
     ...
     try{
      ...
      for(const k of ['captions','captionGroups','captionColor','captionSize','hook','sourceCaptionScenes','captionFadeMs','captionHighlight'])if(JSON.stringify(edl[k])!==JSON.stringify(frozen[k]))throw Error('EDITORIAL_UNSCOPED_CHANGE');
   ...
   const review={...verified,version:QUALITY_VERSION,editorialVersion:EDITORIAL_VERSION,model:profile.director,workflowProfile:profile.version,sha256:ev.sha256,duration:ev.duration,sampleTimes:ev.sampleTimes,captionAudit:verified.captionAudit||null};
  ...
  const plan=validatePlan(await invokeModel({role:'director',name:'direct_edit',schema:planSchema,system:rules+' Inspect the attached source strips in scene order, each showing beginning, middle, end. Direct the best achievable edit from these resources. Return specific decisions for every source. Default to yellow captions, no hook unless requested. If a source has embedded captions, inspect their wording and timing: either explicitly direct sourceCaptionScenes for that source or request source replacement; never stack duplicate captions. Preserve the previous edit and its caption choices when supplied; change only decisions required by the previous review or current request. This is an approved script: never rewrite it.',context:{...context,previousEdit:base?.edl||null,previousReview:base?.review||null},images,usage,env,signal,fetchImpl}),scenes);
  await writeFile(resolve(root,'director-plan.json'),JSON.stringify(plan,null,2));
  let feedback=base?.review?.issues||project.data.creativeMemory?.rejections||null,prior=base?.edl||null;
  for(let round=0;round<2;round++){
   let edl,error=null;
   for(let attempt=0;attempt<2;attempt++){
    const raw=await invokeModel({role:'editor',name:'edit_ad',schema:edlSchema,system:rules+' Execute the director’s approved direction by returning the EDL only. You have scene descriptions and exact word timings, not vision. Use integer frames that sum to outputFrames separately for EACH source. in/out are source seconds. captionGroups are inclusive zero-based word-index pairs, cover every word once in order, usually 1–4 words. Use captionHighlight:true and captionFadeMs:0 for crisp word rhythm. sourceCaptionScenes may only list sources where the director actually observed complete correct embedded captions; omit overlays there, and keep caption groups inside those scene boundaries. captionSize 34–58 at 720px width. Preserve director captions/color/hook exactly. Do not claim to have inspected frames. Correct only concrete validation or reviewer feedback.',context:{...context,plan,previousEdit:prior,feedback,validationError:error},usage,env,signal,fetchImpl});
 ...
 }finally{await writeFile(resolve(root,'usage.json'),JSON.stringify(usage,null,2));if(onUsage)await onUsage(structuredClone(usage));console.log(JSON.stringify({event:'editorial_usage',version:EDITORIAL_VERSION,total:usage.total,calls:usage.calls.length,unknown:usage.unknown,models:[...new Set(usage.calls.map(x=>x.model))]}));}
}

// Public production name; previous import remains available for stored integrations.
export const finishProductionAd=finishSolLunaProject;
