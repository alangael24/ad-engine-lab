...
export function minimaxAlignment(segments,text){
 ...
 if(!words.length||words.map(w=>w.word).join('').replace(/\s/gu,'')!==text.replace(/\s/gu,''))fail();
 let previous=0;
...
export async function synthesizeMiniMax(text,env,{fetchImpl=fetch}={}){
 ...
 return {bytes,alignment,usage:{provider:'minimax',model,characters:d.extra_info?.usage_characters??null,traceId:d.trace_id}};
}
