...
export async function callDirector(project,env,{fetchImpl=fetch,catalog=[],invoke}={}){
 ...
 if(invoke){
  ...
  for(const [i,id] of [project.brand_snapshot.productAssetId,...(project.data.creativeMemory?.referenceAssetIds||[])].entries()){
   const f=await loadImage(id,invoke,fetchImpl);visualEvidence.push({type:'text',text:i===0?'Actual product, authoritative geometry.':'Style reference frame, not product identity.'},{type:'image_url',image_url:{url:`data:${f.mime};base64,${Buffer.from(f.bytes).toString('base64')}`}});
  }
 }
 ...
 const schema={type:'object',additionalProperties:false,properties:{continuity:{type:'string',maxLength:2000},scenes:{type:'array',minItems:1,maxItems:24,items:{type:'object',additionalProperties:false,properties:{text:{type:'string',maxLength:500},visual:{type:'string',maxLength:800},motion:{type:'string',maxLength:400},sourceKey:{type:'string',maxLength:80}},required:['text','visual','motion']}}},required:['continuity','scenes']};
...
export function createProductionProviders(env,{fetchImpl=fetch}={}){
 return {
  ...
  async image({project,plan,index,anchor,previous,invoke}){
   const ids=[project.brand_snapshot.productAssetId,anchor?.assetId,previous?.assetId,...(project.data.creativeMemory?.referenceAssetIds||[]).slice(0,1)].filter((id,i,all)=>id&&all.indexOf(id)===i);
...
export async function reviewImages({project,plan,images,invoke,env,fetchImpl=fetch}){
 if(!project.brand_snapshot?.productAssetId)fail('PRODUCTION_REFERENCE_REQUIRED');
 if(images.length!==plan.scenes.length||images.some(x=>!x.assetId))fail('PRODUCTION_IMAGE_REVIEW_INVALID');
 const scenes=plan.scenes.map((s,i)=>({...s,start:i,end:i+1})),issues=[],usage=[];
 for(const [i,scene] of scenes.entries()){
  ...
  const refs=[['Actual product',project.brand_snapshot?.productAssetId],['Approved style reference',project.data.creativeMemory?.referenceAssetIds?.[0]],['Target still',images[i].assetId],['Previous still for continuity',images[i-1]?.assetId]];
  for(const [label,id] of refs){if(!id)continue;const f=await loadImage(id,invoke,fetchImpl);content.push({type:'text',text:label},{type:'image_url',image_url:{url:`data:${f.mime};base64,${Buffer.from(f.bytes).toString('base64')}`}});}
