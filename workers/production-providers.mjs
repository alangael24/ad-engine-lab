...
export async function callDirector(project,env,{fetchImpl=fetch,catalog=[],invoke}={}){
 ...
 if(invoke){
  if(!isCreatorProduct(project.data)&&!project.brand_snapshot?.productAssetId)fail('PRODUCTION_REFERENCE_REQUIRED');
  for(const [i,id] of [project.brand_snapshot.productAssetId,...(project.data.creativeMemory?.characterAssetIds||[]),...(project.data.creativeMemory?.productViews||[]).map(x=>x.assetId),...(project.data.creativeMemory?.referenceAssetIds||[])].filter(Boolean).slice(0,4).entries()){
   const f=await loadImage(id,invoke,fetchImpl);visualEvidence.push({type:'text',text:i===0&&project.brand_snapshot?.productAssetId?'Actual product, authoritative geometry.':`Additional original reference asset ${id}; its assigned role is in project context.`},{type:'image_url',image_url:{url:`data:${f.mime};base64,${Buffer.from(f.bytes).toString('base64')}`}});
  }
 }
