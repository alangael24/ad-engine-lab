...
def extract_segment(
    ...
    metadata = json.loads(subprocess.check_output(['ffprobe','-v','error','-show_streams','-of','json',str(source)]))
    has_audio = any(x.get('codec_type') == 'audio' for x in metadata['streams'])
    audio_input = [] if has_audio else ['-f','lavfi','-i',f'anullsrc=r=48000:cl=stereo:d={duration}']
    vf_parts: list[str] = []
    ...
    cmd = [
        ...
        "-ss", f"{seg_start:.3f}",
        "-i", str(source),
        *audio_input, "-map", "0:v:0", "-map", "0:a:0" if has_audio else "1:a:0",
        "-t", f"{duration:.3f}",
        "-vf", vf,
        "-af", af,
        "-c:v", "libx264", "-preset", preset, "-crf", crf,
        "-pix_fmt", "yuv420p", "-r", str((output or {}).get("fps", 24)),
...
def extract_all_segments(
    ...
    for i, r in enumerate(ranges):
        ...
        extract_segment(src_path, start, duration, seg_filter, out_path, preview=preview, draft=draft, output=edl.get("output"))
...
def build_master_srt(edl: dict, edit_dir: Path, out_path: Path) -> None:
    ...
    for r in edl["ranges"]:
        ...
        for w in words_in_seg:
            ...
            if len(current) >= edl.get("caption_words", 2) or ends_in_punct:
...
def apply_loudnorm_two_pass(
    ...
    if any(not math.isfinite(float(measurement[k])) for k in ['input_i','input_tp','input_lra','input_thresh']):
...
def main() -> None:
    ...
    if args.no_loudnorm:
        ...
        build_final_composite(base_path, overlays, subs_path, out_path, edit_dir, edl.get("subtitle_style"))
    else:
        # Composite to a temp file, then run loudnorm → final output
        tmp_composite = out_path.with_suffix(".prenorm.mp4")
        build_final_composite(base_path, overlays, subs_path, tmp_composite, edit_dir, edl.get("subtitle_style"))
