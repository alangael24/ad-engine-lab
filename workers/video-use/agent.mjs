...
// Standard Luna <=272K rates, USD per million; cache writes are distinct from hits.
export function estimatedLunaCost(usage){
 const input=usage.input||0,cached=usage.cached||0,writes=usage.cacheWrite||0;
 return (Math.max(0,input-cached-writes)*.2+cached*.02+writes*.25+(usage.output||0)*1.2)/1e6;
}
export function checkEditorBudget(usage,env={}){
 ...
 if(estimatedLunaCost(usage)>=limit||usage.calls>=80||usage.input+usage.output>5000000||usage.unmetered>=2)throw Error('Editing model budget reached');
}
