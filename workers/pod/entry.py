...
os.environ.update(HF_HUB_OFFLINE='0',HF_HUB_CACHE='/workspace/.cache/huggingface',HF_XET_CACHE='/workspace/.cache/xet',HF_XET_HIGH_PERFORMANCE='1',HF_XET_NUM_CONCURRENT_RANGE_GETS='32')
from huggingface_hub import snapshot_download
root=Path('/workspace/ComfyUI')
if not root.exists():shutil.copytree('/opt/ComfyUI',root,symlinks=True)
manifest=json.loads(Path('/opt/creativerush/workers/serverless/model-manifest.json').read_text())
start=time.time()
# Explicit allowlist; never download the whole multi-variant repository.
snapshot_download(manifest['sourceRepo'],revision=manifest['sourceRevision'],allow_patterns=list(manifest['files']),local_dir=str(root/'models'),max_workers=4)
for name,spec in manifest['files'].items():
    if (root/'models'/name).stat().st_size!=spec['bytes']:raise RuntimeError('MODEL_SIZE_MISMATCH')
print(json.dumps({'event':'h3_models_ready','seconds':time.time()-start,'bytes':sum(f['bytes'] for f in manifest['files'].values())}),flush=True)
...
comfy=subprocess.Popen(['python','main.py','--listen','127.0.0.1','--port','8188','--disable-pinned-memory','--fp16-intermediates','--cache-none'],cwd=root)
worker=None
try:
    for _ in range(120):
        if comfy.poll() is not None:raise RuntimeError('COMFY_EXITED')
        try:
