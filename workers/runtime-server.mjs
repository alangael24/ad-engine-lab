...
const processes=['production-worker.mjs','studio-worker.mjs',...(process.env.H3_BACKEND!=='pod'&&process.env.H3_SERVERLESS_ENDPOINT_ID?['h3-worker.mjs']:[]),...(process.env.GPU_IDLE_ENABLED==='true'?['gpu-idle-worker.mjs']:[])];
let stopping=false;const children=new Set();
const server=createServer((req,res)=>{if(req.url!=='/health'||req.method!=='GET'){res.writeHead(404).end();return;}res.writeHead(!stopping&&children.size===processes.length?200:503,{'content-type':'application/json','cache-control':'no-store'}).end(JSON.stringify({status:!stopping&&children.size===processes.length?'ok':'stopping',commit:process.env.RENDER_GIT_COMMIT||null,editorial:process.env.STUDIO_EDITORIAL_ENGINE==='legacy'?'legacy':EDITORIAL_VERSION,workflow:productionWorkflow(process.env).version,models:productionWorkflow(process.env),chatModel:WORKFLOW_MODEL,referenceModel:WORKFLOW_MODEL,scriptModel:SCRIPT_MODEL,clipBufferCapacity:CLIP_BUFFER_CAPACITY,h3PromptWriter:productionPromptModel(process.env),h3PromptGuide:H3_GUIDE_REVISION,imageReview:materialImageReviewEnabled(process.env)?'deepseek-material-v1':'legacy'}));});
