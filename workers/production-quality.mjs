...
import {productionVisionFetch,productionVisionModel,mergeProductionUsage} from '../src/production-vision.js';
...
import {MATERIAL_REVIEW_RULES,QUALITY_VERSION,reviewSchema,validateReview,repeatedSources} from '../assets/quality-model.js';
...
export async function buildReviewEvidence(path,scenes,dir,{signal}={}){
 ...
 for(const [i,s] of scenes.entries()){
  const samples=[s.start+Math.min(1/48,(s.end-s.start)/10),(s.start+s.end)/2,s.end-Math.min(.05,(s.end-s.start)/10)],hs=[];sampleTimes.push({sceneId:s.id,times:samples});
  for(const [k,t] of samples.entries()){
   const f=join(dir,`frame-${i}-${k}.jpg`),raw=join(dir,`hash-${i}-${k}.gray`);
   await command('ffmpeg',['-v','error','-y','-ss',String(t),'-i',path,'-frames:v','1','-vf',`scale=240:426:force_original_aspect_ratio=decrease,pad=240:450:(ow-iw)/2:24,drawtext=fontfile=${font}:text='S${i+1} ${t.toFixed(2)}s':x=5:y=4:fontsize=16:fontcolor=white`,'-pix_fmt','yuvj420p','-update','1',f],{signal});
 ...
 for(let start=0;start<thumbs.length;start+=18){
  ...
  await command('ffmpeg',['-v','error','-y','-f','concat','-safe','0','-i',list,'-vf',`tile=3x${Math.ceil(files.length/3)}:nb_frames=${files.length}:padding=12:margin=12:color=gray`,'-frames:v','1','-pix_fmt','yuvj420p','-update','1',out],{signal});sheets.push(out);
 }
...
async function requestReview(system,content,env,fetchImpl,signal){
 const r=await ((f,u,i)=>productionVisionFetch(f,u,i,env))(fetchImpl,'https://opencode.ai/zen/go/v1/chat/completions',{method:'POST',headers:{authorization:`Bearer ${env.REFERENCE_FLASH_KEY}`,'content-type':'application/json'},body:JSON.stringify({model:productionVisionModel(env),reasoning_effort:'none',stream:true,stream_options:{include_usage:true},max_tokens:6500,tool_choice:{type:'function',function:{name:'review_ad'}},tools:[{type:'function',function:{name:'review_ad',parameters:reviewSchema}}],messages:[{role:'system',content:system},{role:'user',content}]}),signal:signal?AbortSignal.any([signal,AbortSignal.timeout(900000)]):AbortSignal.timeout(900000)});
  ...
  if(model!==productionVisionModel(env)||finish!=='tool_calls'||calls.size!==1||name!=='review_ad')throw Object.assign(Error('PRODUCTION_QUALITY_INVALID'),{detail:'stream_contract'});
...
export async function askReview(system,content,env,fetchImpl,signal,scenes,targetSceneId){
 ...
 for(let attempt=0;attempt<2;attempt++){
  try{
   ...
   usage=mergeProductionUsage(usage,result.usage);
...
export async function reviewVideo({path,scenes,project={},renderId,env=process.env,directory,fetchImpl=fetch,signal}){
 if(!(env.OPENAI_API_KEY||env.EDITORIAL_ASTRA_API_KEY||env.REFERENCE_FLASH_KEY))throw Error('PRODUCTION_QUALITY_OFFLINE');
 ...
 try{
  ...
  for(const [n,issue] of unique.slice(0,6).entries()){
   const previous=issue.kind==='continuity'?scenes[scenes.findIndex(s=>s.id===issue.sceneId)-1]?.id:null;
   const ids=[...new Set([issue.sceneId,issue.relatedSceneId||previous].filter(Boolean))],files=[];
   ...
   await writeFile(list,files.map(f=>`file '${f.replaceAll("'","'\\''")}'`).join('\n'));
   await command('ffmpeg',['-v','error','-y','-f','concat','-safe','0','-i',list,'-vf',`tile=3x${ids.length}:nb_frames=${files.length}:padding=12:margin=12:color=gray`,'-frames:v','1','-pix_fmt','yuvj420p','-update','1',out],{signal});
   ...
   const focused=[{type:'text',text:JSON.stringify({...detailContext,instruction:detailContext.instruction+' Each following image is ONE separate actual frame, with scene/time label. Do not treat neighboring images as one room. Verify alleged defects against these individual frames before spending on replacements.'})}];
   for(const id of ids){
    const index=scenes.findIndex(s=>s.id===id),times=[...new Set([...evidence.sampleTimes[index].times,...(id===issue.sceneId?[Math.min(issue.at,scenes[index].end-Math.min(.05,(scenes[index].end-scenes[index].start)/10))]:[])])].sort((a,b)=>a-b);
    for(const [k,t] of times.entries()){
     const individual=join(dir,`individual-${n}-${index}-${k}.jpg`);
     await command('ffmpeg',['-v','error','-y','-ss',String(t),'-i',path,'-frames:v','1','-vf','scale=720:1280:force_original_aspect_ratio=decrease','-pix_fmt','yuvj420p','-update','1',individual],{signal});
     focused.push({type:'text',text:`Scene ${id}, actual output time ${t.toFixed(4)}s`},{type:'image_url',image_url:{url:'data:image/jpeg;base64,'+(await readFile(individual)).toString('base64')}});
    }
   }
   const check=await askReview(system+'\nIndependently confirm ONE proposed defect using the individual frames. Do not trust the previous review.',focused,env,fetchImpl,signal,scenes,issue.sceneId);
   usage=mergeProductionUsage(usage,check.usage);
...
export async function reviewProduction({renderId,invoke,env,fetchImpl=fetch}){
 ...
 try{
  const r=await fetchImpl(media.url,{redirect:'error',signal:AbortSignal.timeout(900000)});if(!r.ok)throw Error('PRODUCTION_QUALITY_MEDIA');
