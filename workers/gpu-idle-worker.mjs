...
// Stop-only: never create, resume, resize or destroy a pod. Defaults off.
export async function checkGpuIdle(env,{fetchImpl=fetch}={}){
 ...
 if(state.desiredStatus!=='RUNNING')return {status:'not_running'};
 ...
 const stopped=await runpod('/stop','POST');
 if(!stopped.ok)throw Error('GPU_STOP_UNCONFIRMED');
