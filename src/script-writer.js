...
export async function writeScriptChanges(raw,{project,history,message,env,request=fetch,onDelta}){
 ...
 for(const change of changes||[]){
  ...
  const r=await request('https://opencode.ai/zen/go/v1/chat/completions',{method:'POST',headers:openCodeHeaders({authorization:`Bearer ${env.REFERENCE_FLASH_KEY}`,'content-type':'application/json','x-opencode-session':'creativerush-script-'+project.id}),signal:AbortSignal.timeout(120000),body:JSON.stringify({model:SCRIPT_MODEL,reasoning_effort:'none',stream:true,stream_options:{include_usage:true},max_tokens:4500,tool_choice:{type:'function',function:{name:'write_script'}},tools:[{type:'function',function:{name:'write_script',parameters:{type:'object',additionalProperties:false,properties:{value:{type:'string',...(bounded?{maxLength:Math.ceil(maxWords*6.5)}:{})}},required:['value']}}}],messages:[{role:'system',content:isCreatorProduct(project.data)?CREATOR_SCRIPT_SYSTEM+(bounded?`\nLÍMITE DE PRODUCCIÓN: apunta a ${Math.floor(maxWords*.78)} palabras, NUNCA más de ${maxWords}. Es un video de ${project.data.creatorBrief?.targetDuration||30} segundos. Cuenta las palabras antes de entregar. Si hay un borrador largo, resume su idea en 4–6 frases cortas: premisa, un intento y remate. No desarrolles cuatro intentos ni repitas introducciones. Esta solicitud autoriza esa reescritura; no es una conservación literal. ${durationFeedback}`:''):isSalesProduct(project.data)?SALES_COPY_SYSTEM:LEGACY_SCRIPT_SYSTEM},{role:'user',content:JSON.stringify({...(bounded?{lengthContract:{targetWords:Math.floor(maxWords*.78),maximumWords:maxWords,targetSeconds:project.data.creatorBrief?.targetDuration||30},...(previousDraft?{rejectedDraft:previousDraft}: {})}:{}),request:message,operation:change.operation,sceneId:change.sceneId,editorialInstruction:change.value,brand:project.brand_snapshot,data:project.data,history:history.slice(-8)})}]})});
