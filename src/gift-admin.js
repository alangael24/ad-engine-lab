...
const summary=o=>({id:o.id,title:o.brief?.title||'Película personalizada',status:o.status,needsPersonalization:o.personalization?.state==='pending',targetSeconds:o.target_seconds,reservedSeconds:o.reserved_seconds,deliveredSeconds:o.delivered_seconds,createdAt:o.created_at,updatedAt:o.updated_at});
export async function getGiftAdmin(context){try{
 ...
 let q=db.from('gift_orders').select('id,brief,personalization,status,target_seconds,reserved_seconds,delivered_seconds,created_at,updated_at').order('created_at',{ascending:false}).order('id',{ascending:false}).range(page*50,page*50+50);
