...
export async function getStudio(context){try{
 ...
 if(url.searchParams.has('render')){const r=await own(db,'studio_renders',user.id,url.searchParams.get('render'));if(r.status!=='succeeded')fail('STUDIO_NOT_READY');return json({url:await signed(db,'studio-media',r.result_path,url.searchParams.has('download'))});}
...
export async function postStudio(context){try{
 ...
 if(body.action==='save_brand')data=brandData(data);
  ...
  const old=await db.from('studio_scene_versions').select('project_id,request_payload').eq('user_id',user.id).eq('request_id',data.requestId).maybeSingle();if(old.error)throw old.error;
  if(old.data){
   const previous=old.data.request_payload;
   if(old.data.project_id!==body.id||previous.sceneId!==data.sceneId||previous.assetId!==(data.assetId||null)||previous.instruction!==data.instruction)throw new ApiError('IDEMPOTENCY_CONFLICT');
   data=previous;
  }else{
   if(!data.assetId&&context.env.GENERATION_ENABLED!=='true')throw new ApiError('WORKER_OFFLINE');
   const p=await own(db,'studio_projects',user.id,body.id);
   data={requestId:data.requestId,sceneId:data.sceneId,assetId:data.assetId||null,instruction:data.instruction,prompt:scenePrompt(p,data.sceneId,data.instruction)};
  }
