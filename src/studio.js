...
export async function postStudio(context){try{
 ...
 if(body.action==='save_brand')data=brandData(data);
  ...
  data=projectData(data);if(body.action==='save_project'){const prior=await own(db,'studio_projects',user.id,body.id);if(productProfile(prior.data)!==productProfile(data))fail('STUDIO_PRODUCT_LOCKED');}if(body.action==='create_project'&&(data.scenes.length||data.narrationAssetId||data.referenceAnalysisId))fail();
