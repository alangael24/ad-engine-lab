...
export async function getGiftOrder(context){try{
 ...
 return json({order:publicOrder(o),seconds:guest?{available:o.target_seconds}:await secondsBalance(db,user.id),canAdmin:!guest&&isGiftAdmin(user,context.env),guest:Boolean(guest)});
}catch(e){return error(e);}}
export async function postGiftOrder(context){try{
 const b=await readJson(context.request,4000);
