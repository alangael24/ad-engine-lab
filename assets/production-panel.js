...
export function productionPanel({api,task,getProject,isDirty,tell,refresh,getRenders=()=>[]}){
 ...
 async function open(p){
  ...
  button.hidden=Boolean(active)||!r.enabled;button.disabled=!r.enabled||!p.data.scriptDraft?.trim();button.textContent=j?.status==='succeeded'?'Crear otra versión':'Aprobar guion y crear video';
  ...
  if(!active&&render)status.textContent=render.status==='succeeded'?'Tu video está listo.':render.status==='failed'?'No pudimos terminar el montaje. Puedes pedir que lo intentemos de nuevo.':'Montando tu video…';
