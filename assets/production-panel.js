...
export function productionPanel({api,task,getProject,isDirty,tell,refresh,getRenders=()=>[]}){
 ...
 async function open(p){
  ...
  if(!active&&render)status.textContent=render.status==='succeeded'?'Tu video está listo.':render.status==='quality_failed'?'El video necesita ajustes antes de entregarlo.':render.status==='reviewing'?'Revisando tu video…':render.status==='failed'?'No pudimos terminar el montaje. Puedes pedir que lo intentemos de nuevo.':'Montando tu video…';
