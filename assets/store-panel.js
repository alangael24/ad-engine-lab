...
export function storePanel({api,task,tell,getBrand,changed}){
 ...
 async function choose(p){
  const current=epoch;
  if(p.description===undefined||(sales&&!p.salesSource)){const details=await api('/api/store-import',{method:'POST',body:{action:'inspect',url:p.url,...(sales?{productProfile:'ads-sales-v1'}:{})}});if(current!==epoch)return;p=details.products.find(x=>x.description!==undefined)||null;if(!p){show();throw Error('No encontramos la descripción. Escribe qué producto quieres anunciar.');}}
  ...
  delete brand.data.salesSource;if(p.salesSource)brand.data.salesSource=p.salesSource;
