...
const fail=()=>{throw Error('PRODUCTION_QUALITY_INVALID');};
const text=(s,max)=>{if(typeof s!=='string'||!s.trim()||s.length>max)fail();return s.trim();};
export const reviewSchema={type:'object',additionalProperties:false,properties:{verdict:{type:'string',enum:['pass','repair','blocked']},summary:{type:'string'},issues:{type:'array',maxItems:24,items:{type:'object',additionalProperties:false,properties:{sceneId:{type:'string'},relatedSceneId:{type:['string','null']},kind:{type:'string',enum:['repetition','mismatch','continuity','artifact','caption','timing']},at:{type:'number'},evidence:{type:'string'},action:{type:'string',enum:['replace_clip','replace_image','none']},visual:{type:'string'},motion:{type:'string'}},required:['sceneId','relatedSceneId','kind','at','evidence','action','visual','motion']}}},required:['verdict','summary','issues']};
export function validateReview(raw,scenes){
 ...
 const issues=raw.issues.map(i=>{
  ...
  if(!s||!['repetition','mismatch','continuity','artifact','caption','timing'].includes(i.kind)||!Number.isFinite(i.at)||i.at<s.start||i.at>s.end||!['replace_clip','replace_image','none'].includes(i.action)||(i.relatedSceneId!=null&&(!scenes.some(s=>s.id===i.relatedSceneId)||i.relatedSceneId===s.id)))fail();
  if(['caption','timing'].includes(i.kind)&&i.action!=='none')fail();
 ...
 if((raw.verdict==='pass')!==(!issues.length)||raw.verdict==='repair'&&issues.some(i=>i.action==='none'))fail();
 const actions=issues.filter(i=>i.action!=='none');if(new Set(actions.map(i=>i.sceneId)).size!==actions.length)fail();
