...
export function normalizeCreativeMemory(raw={}) {
 ...
 return {version:2,characterAssetIds:list(raw.characterAssetIds,3,uuid),productViews:list(raw.productViews,6,x=>x&&uuid(x.assetId)&&text(x.features)),observedStates:list(raw.observedStates,24,x=>x&&uuid(x.sceneId)&&uuid(x.assetId)&&text(x.summary)),referenceAssetIds:list(raw.referenceAssetIds,3,uuid),preferences:list(raw.preferences,30,text),decisions:list(raw.decisions,50,text),rejections:list(raw.rejections,50,text),approvedAssets:list(raw.approvedAssets,24,uuid)};
}
export function remember(project,{decision,rejection,approvedAssets,observedStates,rejectedAssets=[]}={}) {
 ...
 m.approvedAssets=m.approvedAssets.filter(x=>!rejectedAssets.includes(x));
 if(observedStates)m.observedStates=[...m.observedStates.filter(x=>!observedStates.some(y=>y.sceneId===x.sceneId)),...observedStates].filter(x=>m.approvedAssets.includes(x.assetId)).slice(-24);
 m.observedStates=m.observedStates.filter(x=>m.approvedAssets.includes(x.assetId));
