...
async function load(){if(busy)return;busy=true;$('#refresh').disabled=true;$('#notice').textContent='Cargando pedidos…';$('#authorized').hidden=true;$('#login').hidden=true;$('#switch-account').hidden=true;
 try{
  ...
  if(!id){$('#orders').replaceChildren();$('#page-label').textContent=`Página ${page+1}`;$('#previous').disabled=page===0;$('#next').disabled=!r.hasMore;
   ...
   for(const o of r.orders){const a=el('a','','admin-card');a.href='/regalos/admin/?id='+o.id;a.append(el('h2',o.title),el('span',states[o.status],'admin-badge'),el('p',`${o.targetSeconds/60} min · ${o.targetSeconds===120?'Entrega prioritaria':'Estándar · 24 h'} · ${date(o.createdAt)} · ${o.id.slice(0,8)}`));$('#orders').append(a);}return;
  }
  const o=r.order;$('#detail-title').textContent=o.title;$('#facts').replaceChildren();
  for(const [k,v]of [['Pedido',o.id],['Cliente',o.customerEmail||'Correo no disponible'],['Estado',states[o.status]],['Duración',o.targetSeconds/60+' minuto(s)'],['Entrega',o.targetSeconds===120?'Prioritaria':'Estándar · 24 horas'],['Formato',o.brief.aspectRatio],['Estilo',o.brief.creative?.look==='clay'?'Plastilina':o.brief.creative?.look==='3d'?'Animación 3D':o.brief.creative?.look||'—'],['Recibido',date(o.createdAt)],['Último cambio',date(o.updatedAt)],['Guion aprobado',date(o.approvedAt)],['Entregado',date(o.deliveredAt)],['Duración reservada',o.reservedSeconds+' s'],['Duración entregada',o.deliveredSeconds==null?'—':o.deliveredSeconds+' s']])fact(k,v);
