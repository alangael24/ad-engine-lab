...
async function refresh(){loaded=false;covered=false;say('');$('#checkout-pay').disabled=true;try{const auth=await getAuthClient();const {data,error}=await auth.auth.getSession();if(error)throw error;session=data.session;
