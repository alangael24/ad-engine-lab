...
function show(n){step=n;document.body.dataset.giftStep=String(n);panels.forEach((p,i)=>p.hidden=i!==n);document.querySelectorAll('[data-step]').forEach(b=>b.setAttribute('aria-current',Number(b.dataset.step)===n?'step':'false'));$('#form-title').textContent=stepTitles[n];$('#gift-back').hidden=n===0;$('#gift-next').hidden=n===3;$('#gift-purchase').hidden=n!==3||guest;$('#gift-next').textContent=['Recordar juntos →','Añadir sus fotos →','Elegir la duración →'][n]||'Continuar →';$('#gift-duration').hidden=n!==3;const f=fields();$('#gift-summary').textContent=`Para ${f.recipient||'esa persona especial'} · ${GIFT_OCCASIONS[f.occasion]||'Su historia'} · ${GIFT_EMOTIONS[f.emotion]||'Amor'} · Animación 3D tipo Pixar.`;saveDraft();}
