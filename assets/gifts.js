...
async function refreshAuth(){try{const auth=await getAuthClient();const {data,error}=await auth.auth.getSession();if(error)throw error;session=data.session;$('#gift-photos').disabled=busy||!session;$('#photo-auth').replaceChildren();if(!session){const a=document.createElement('a');a.href='/cuenta/?next=regalos';a.textContent='Inicia sesión para añadir tus fotos.';a.onclick=saveDraft;$('#photo-auth').append(a);}else{$('#account-link').href='/crear/';$('#account-link').textContent='Mi estudio';}return session;}catch(e){status('No pudimos conectar tu cuenta. Puedes escribir tu historia y volver a intentarlo.',true);return null;}}
