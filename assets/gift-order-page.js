...
async function refresh(){clearTimeout(timer);try{
 ...
 order=r.order;$('#order-whatsapp').hidden=!r.whatsapp;if(r.whatsapp&&!whatsappLoaded){$('#whatsapp-consent').checked=Boolean(r.whatsapp.phone);$('#whatsapp-number').value=r.whatsapp.phone||'';paintWhatsApp();whatsappLoaded=true;}$('#order-title').textContent=order.title;$('#order-details').hidden=false;say(states[order.status]);$('#order-reference').textContent='Pedido '+order.id.slice(0,8);$('#order-progress').textContent=`Película de ${order.targetSeconds/60} minuto${order.targetSeconds>60?'s':''}`;
