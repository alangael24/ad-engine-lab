...
## Live response streaming (2026-09-05)

The chat API accepts `Accept: text/event-stream` after authentication and reservation. It forwards only decoded message and narration text deltas from the provider tool arguments. It validates the complete tool output before committing. JSON clients and idempotent replay retain their original response shape. A disconnect does not start another request; the worker completion is registered through waitUntil when available. The client reveals complete words directly from incoming text and replaces the preview with the persisted result; it has no simulated typing timer.

Tests cover fragmented UTF-8, escaped JSON strings, unfinished responses, early preview before completion, validation before write, and client disconnect followed by idempotent replay. The local real-provider fixture supports SSE passthrough and optional STUDIO_PREVIEW_PORT / STUDIO_PREVIEW_OUTPUT to preserve an existing test session. The large style cards are replaced by compact text choices.

## One-input start (2026-09-05)

The home screen contains one idea/link input, a reference attachment button, and submit. The current product is a small changeable label; optional style and editing tools live under an overflow menu. The chat uses the same compact input. Saved-product selection is remembered locally and checked against the authenticated library before reuse.

A store/product link invokes the existing importer directly. A single product is saved with its selected image and opens a conversation; several products require only one inline choice. An idea entered before product setup is retained and used after import. Social/video links are stored as references, not sent to the store importer or represented as watched. Reference uploads keep the existing automatic analysis path and unavailable-state handling. Manual product entry remains available as a fallback.

Verification: routing and brand-contract tests, existing creative/streaming tests (9 passing), and browser interaction against `tests/simple-studio-preview.mjs` (local-only mock providers). The browser flow confirmed idea preservation, automatic single-product continuation, and correct selection from several products. The current port-4208 real-chat demo still has store import and reference analysis disabled; these controls do not enable new providers.

For accounts without a saved product, the main heading asks which product to advertise and the input requests a store URL. Reference/style controls appear only after product setup. A URL alone saves the product and advances to the idea question without starting an unsolicited generic script. A previously supplied idea still continues automatically. Verified both headings and the transition in the local browser fixture.
