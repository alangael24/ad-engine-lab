...
The current MCP endpoint-update schema does not expose the cached-model field; it was changed through the authenticated Chrome console. Do not use the publishing credential on Runpod or assume an env change alone selects the cached model. Keep the endpoint at min/max zero until an authorized preparation run with a verified shutdown path. Selecting a repository alone is not proof that Runpod has cached its weights on an available host. A controlled first-worker startup and shutdown measurement remains pending; the CPU bridge's separate control-credential/deployment requirement above also remains unresolved.
