...
const paths=[...new Set((await Promise.all(['assets','src','workers','functions/api'].map(files))).flat().concat(['scripts/freeze-production-workflow.mjs','scripts/build-static.mjs','scripts/build-pages-worker.mjs','package.json','package-lock.json','wrangler.toml']))].sort();
